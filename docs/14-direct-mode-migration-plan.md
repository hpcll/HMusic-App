...
## 纯播放器与 LX/Spotify 修复（2026-09-18，未发布）

- 新增 `player` 模式：从连接/设置入口选择后与冷启动均进入首页（榜单 `/charts`）；
  按用户后续反馈取消首次强制进入音源设置，音源配置仍从设置手动进入。
  无 Server 或小米登录要求，设备固定本机；旧音箱选择仍保留给直连，但不能在纯播放器调用。
  搜索、解析、歌词、队列、歌单、收藏、统计复用本地仓库与同一个 AudioHandler。
- 纯播放器与直连共享 `hmusic.direct.v1` 数据，而不是各建一份曲库；Server 数据与凭据仍独立。
  切换暂停并保留进度，切回不自动开播；目标能力在暂停前校验。
  StoreEdition 固定 Server；Windows/Linux 本机能力未开放时隐藏入口、拒绝选择，旧偏好回退 Server。
- Android `flutter_js 0.8.7` 缺失 `jsSetMemoryLimit` 包装导出，原带 memoryLimit 的构造失败。
  `lx_javascript_engine.dart` 在 Android 创建引擎后、执行用户脚本前调用已导出的
  `JS_SetMemoryLimit`，保留 64 MB 上限；Apple 保持 JSC，未改 pub cache 或第三方依赖。
- Spotify 公开嵌入页面请求总预算 8 秒，超时取消请求；错误明确提示网络不可达。
  成功的榜单元数据持久缓存一天，过期刷新失败时显示保存日期；无缓存则显示可重试错误，
  不伪造空榜、不把预览 URL 当整曲。该修复不保证第三方网络可达，也不新增个人账号功能。
- 关于页网盘入口改为下载图标 + 描边按钮，最小高度 48；iOS 仍不展示安装包网盘入口。

本轮结果与日志索引见 [验收记录](reviews/2026-09-18-player-lx-spotify.md)。
模拟器覆盖原生链路，不能替代 Android/iOS 真机后台、锁屏长时、真实小米账号/音箱验收。

...
- **★ 窄屏单屏模式（≤1023，移动端规范交互，Flutter 照此实现）**：
  ...
  下一行 timeMs 的占比；web 用 gradient+background-clip:text+@property 过渡，Flutter 用
  ShaderMask/自绘）；无歌词时显示「暂无歌词」。歌词本体即入口，不加额外按钮装饰。
...
## 屏 2b · 歌词页 lyrics.js（#/lyrics，窄屏专用沉浸式路由）
...
- **进入/退出**：播放页点封面或歌词条进入；收起键 `go("player")` 或**系统返回手势**退出
  （真路由带 hash 历史，返回手势白送——这是"页面优于浮层"的核心理由）。
- **状态**：与播放页共享 `lyric-state.js` 歌词缓存（同曲不重复请求）；自带 1s 本地插值 +
  5s 服务端校准（与播放页同款双定时器）。
- **交互→API**：三键 → `POST /playback/{previous|pause|resume|next}`（本机播放先 localPlay/localPause）；
  进度拖动/行点 → `POST /playback/seek`（本机再 localSeek）。
